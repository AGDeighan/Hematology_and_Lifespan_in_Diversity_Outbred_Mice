...
################################################################################
#
#   This script estimates the effect of sex on each of the CBC traits. 
# 
  ...
  if(TRAIT %in% Clump_Affected_Traits){
    ...
    FULL_MODEL <- tryCatch(
      suppressMessages(lmer(
        Y ~ (1|Generation) + (1|DOT) + (1|MouseID) + clumps + AgeAtTest + Sex,
        REML = FALSE,
    ...
    NULL_MODEL <- tryCatch(
      suppressMessages(lmer(
        Y ~ (1|Generation) + (1|DOT) + (1|MouseID) + clumps + AgeAtTest,
        REML = FALSE,
        control = lmerControl(optimizer = "bobyqa"),
        data = RAW_DATA %>% mutate(Y = rankZ(Y))
      )),
    ...
    FULL_MODEL <- tryCatch(
      suppressMessages(lmer(
        Y ~ (1|Generation) + (1|DOT) + (1|MouseID) + AgeAtTest + Sex,
        REML = FALSE,
        control = lmerControl(optimizer = "bobyqa"),
        data = RAW_DATA %>% mutate(Y = rankZ(Y))
      )),
    ...
    NULL_MODEL <- tryCatch(
      suppressMessages(lmer(
        Y ~ (1|Generation) + (1|DOT) + (1|MouseID) + AgeAtTest,
        REML = FALSE,
        control = lmerControl(optimizer = "bobyqa"),
        data = RAW_DATA %>% mutate(Y = rankZ(Y))
      )),
...
# Generation     (Intercept) 7.4230e-08
# Residual                   6.1482e-01
#
# -------------------- #
#
...
# Generation:DOT (Intercept) 0.21666
...
# Generation:DOT (Intercept) 0.23185
# Generation     (Intercept) 0.00000
# Residual                   0.62582
#
# -------------------- #
...
SEX_EFFECTS <- SEX_EFFECTS %>%
  mutate(
    ...
    # The phenotypes were standerdised during model fitting, so to get 
    # the sex effects and standard errors in units of traits, we need to 
    # multiply them by the standard deviation of the trait
    Effect = Effect * TraitSD,
    SE = SE * TraitSD
  ) %>%
  select(
...
# write_csv(
#   SEX_EFFECTS,
#   'tables/lifespan_aging_hematology/sex_effects_on_hematology_traits_e7.csv'
# )
